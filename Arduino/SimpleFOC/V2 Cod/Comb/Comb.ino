...
void setup() {
 ...
 motor.init();
  ...
  // motor.zero_electric_angle=5.01;
  ...
  // jerk control using voltage voltage ramp
  // default value is 300 volts per sec  ~ 0.3V per millisecond
  motor2.PID_velocity.output_ramp = 1100;
...
void loop() {
 ...
 motor2.loopFOC();
  ...
  if(last_error_pitch != 0)
    error_pitch =0;
  if(error_pitch != 0){
    move_pitch= -error_pitch/57; ///57.4; // 56.7
  }
 ...
 target_angle_pitch += move_pitch;
  ...
  //    Serial.print("Error:"); Serial.print(error_pitch);Serial.println("");
